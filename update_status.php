...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == 'accept') {
        ...
        $updateStatusQuery = "UPDATE daftardonor SET status = 'Selesai' WHERE id = $id";
        $addPointsQuery = "UPDATE users SET poin = poin + 100 WHERE nama = 
            (SELECT nama FROM daftardonor WHERE id = $id)";

        if ($conn->query($updateStatusQuery) === TRUE && $conn->query($addPointsQuery) === TRUE) {
        ...
        $updateStatusQuery = "UPDATE daftardonor SET status = 'Gagal' WHERE id = $id";
